...
function xmldb_plagiarism_essayguard_upgrade($oldversion) {
    // V1.2.220: $CFG was used further down without being declared global, so on PHP 8 the
    // require_once safety net silently never ran and the upgrade printed warnings.
    global $DB, $CFG;
    ...
    if ($oldversion < 2026030600) {
        ...
        // That is a CORE site setting (Site administration > Advanced features), not a
        // plugin setting. Installing or upgrading a plagiarism plugin silently switched
        // on Moodle's whole plagiarism subsystem for the entire site, activating every
        // other plagiarism plugin installed alongside it and overriding a deliberate
        // administrator decision to leave it off. A plugin may read a core setting; it
        // must never write one. The admin is now told what to do instead — see the
        // notice raised by settings.php / the post-install message.
        upgrade_plugin_savepoint(true, 2026030600, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026030700) {
        ...
        if (!$dbman->table_exists($fptable)) {
            $fptable->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
            ...
            $fptable->add_field('baseline_wpm', XMLDB_TYPE_NUMBER, '8', null, XMLDB_NOTNULL, null, '0', null, '2');
            $fptable->add_field('baseline_pause_mean', XMLDB_TYPE_NUMBER, '10', null, XMLDB_NOTNULL, null, '0', null, '2');
            $fptable->add_field('baseline_backspace_ratio', XMLDB_TYPE_NUMBER, '8', null, XMLDB_NOTNULL, null, '0', null, '4');
            $fptable->add_field('baseline_burst_mean', XMLDB_TYPE_NUMBER, '10', null, XMLDB_NOTNULL, null, '0', null, '2');
            $fptable->add_field('baseline_sentence_variance', XMLDB_TYPE_NUMBER, '10', null, XMLDB_NOTNULL, null, '0', null, '4');
            $fptable->add_field('baseline_vocab_diversity', XMLDB_TYPE_NUMBER, '8', null, XMLDB_NOTNULL, null, '0', null, '4');
            $fptable->add_field('baseline_entropy', XMLDB_TYPE_NUMBER, '8', null, XMLDB_NOTNULL, null, '0', null, '4');
            $fptable->add_field('baseline_interkey_mean', XMLDB_TYPE_NUMBER, '10', null, XMLDB_NOTNULL, null, '0', null, '2');
    ...
    if ($oldversion < 2026031200) {
        upgrade_plugin_savepoint(true, 2026031200, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026031600) {
        upgrade_plugin_savepoint(true, 2026031600, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026031700) {
        ...
        upgrade_plugin_savepoint(true, 2026031700, 'plagiarism', 'essayguard');
    }
    ...
    // v1.2.9 (2026031800) is added here so the DB version advances past v1.2.9
    // for any site that had the "upgrade required" loop.  No DB schema changes.
    if ($oldversion < 2026031800) {
        upgrade_plugin_savepoint(true, 2026031800, 'plagiarism', 'essayguard');
    }
    ...
    // was added by upgrade.php at v1.2.14 (savepoint 2026032001). Sites
    ...
    // step at 2026032001 already handles the column for upgraded sites.
    //
    ...
    // 2026032001 added the column but never updated the index. When the
    ...
    if ($oldversion < 2026032001) {
        ...
        // 2026032001 for upgraded sites; needed here for any edge-case
    ...
    if ($oldversion < 2026032601) {
        ...
        $orphansql = "SELECT DISTINCT ev.userid, ev.cmid, ev.contextid, ev.attemptkey
        ...
        $orphans = $DB->get_records_sql($orphansql);
        ...
        if (!empty($orphans)) {
            ...
            foreach ($orphans as $orphan) {
                try {
                    ...
                    debugging(
                        '[plagiarism_essayguard] retroactive score failed'
            ...
            debugging(
                '[plagiarism_essayguard] v1.2.25 retroactive scoring complete:'
    ...
    if ($oldversion < 2026032700) {
        upgrade_plugin_savepoint(true, 2026032700, 'plagiarism', 'essayguard');
    }
    ...
    // v1.2.28 (2026032700). Sites upgrading from v1.2.27 directly to
    ...
    if ($oldversion < 2026032800) {
        upgrade_plugin_savepoint(true, 2026032800, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026033000) {
        upgrade_plugin_savepoint(true, 2026033000, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026033100) {
        upgrade_plugin_savepoint(true, 2026033100, 'plagiarism', 'essayguard');
    }
    ...
    // version.php → 2026033103.
    if ($oldversion < 2026033103) {
        upgrade_plugin_savepoint(true, 2026033103, 'plagiarism', 'essayguard');
    }
    ...
    // carried over from v1.2.36. version.php → 2026033103.
    if ($oldversion < 2026033103) {
        upgrade_plugin_savepoint(true, 2026033103, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. AMD: tracker.js updated. version.php → 2026040200.
    if ($oldversion < 2026040200) {
        upgrade_plugin_savepoint(true, 2026040200, 'plagiarism', 'essayguard');
    }
    ...
    //
    // No DB schema changes. version.php → 2026040200.
    if ($oldversion < 2026040200) {
        upgrade_plugin_savepoint(true, 2026040200, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. version.php → 2026040300.
    if ($oldversion < 2026040300) {
        upgrade_plugin_savepoint(true, 2026040300, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. version.php → 2026040700.
    if ($oldversion < 2026040700) {
        upgrade_plugin_savepoint(true, 2026040700, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. version.php → 2026040800.
    if ($oldversion < 2026040800) {
        upgrade_plugin_savepoint(true, 2026040800, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. JS-only: amd/src/tracker.js. version.php → 2026040900.
    // ⚠ FORMAT BUG: 2026040900 is 12-digit. It is numerically LESS than v1.2.44's
    // 2026040800 (13-digit). Any site with $oldversion >= 2026040800 will skip
    // this block. No DB changes here so the skip is harmless. Fixed in v1.2.47.
    if ($oldversion < 2026040900) {
        upgrade_plugin_savepoint(true, 2026040900, 'plagiarism', 'essayguard');
    }
    ...
    // No new code changes. No DB schema changes. version.php → 2026040900.
    // ⚠ FORMAT BUG: 2026040900 is 12-digit. Same issue as v1.2.45 above — skipped
    // by any site with $oldversion >= 2026040800. Harmless. Fixed in v1.2.47.
    if ($oldversion < 2026040900) {
        upgrade_plugin_savepoint(true, 2026040900, 'plagiarism', 'essayguard');
    }
    ...
    // numeric version (2026040800) was larger than the 12-digit v1.2.45/v1.2.46
    // numerics (2026040900/2026040900). Moodle rejected those ZIPs as "downgrade".
    // No code changes. No DB schema changes. version.php → 2026041000.
    if ($oldversion < 2026041000) {
        upgrade_plugin_savepoint(true, 2026041000, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. version.php → 2026041000.
    if ($oldversion < 2026041000) {
        upgrade_plugin_savepoint(true, 2026041000, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. version.php → 2026041100.
    if ($oldversion < 2026041100) {
        upgrade_plugin_savepoint(true, 2026041100, 'plagiarism', 'essayguard');
    }
    ...
    // No DB changes. version.php → 2026041300.
    if ($oldversion < 2026041300) {
        upgrade_plugin_savepoint(true, 2026041300, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026041400) {
        upgrade_plugin_savepoint(true, 2026041400, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026041500) {
        upgrade_plugin_savepoint(true, 2026041500, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026041700) {
        ...
        upgrade_plugin_savepoint(true, 2026041700, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. version.php → 2026042000.
    if ($oldversion < 2026042000) {
        upgrade_plugin_savepoint(true, 2026042000, 'plagiarism', 'essayguard');
    }
    ...
    // No DB schema changes. version.php → 2026042100.
    if ($oldversion < 2026042100) {
        upgrade_plugin_savepoint(true, 2026042100, 'plagiarism', 'essayguard');
    }
    ...
    // V1.2.70: AMD ENCODING FIX: All non-ASCII characters (em dashes, arrows, box-drawing chars, ellipsis,
    // bullets, emoji, accented Latin) scrubbed from all AMD JS files (amd/src, amd/build,
    // amd/build/*.min.js). Root cause of Moodle primary/secondary navigation menus disappearing site-wide:
    // non-ASCII bytes in any installed plugin's AMD file cause a SyntaxError inside RequireJS's first.js
    // bundle, throwing "No define call for core/first" and aborting the entire AMD module chain. No PHP, DB
    // schema, or functional changes in this release.
    if ($oldversion < 2026042200) {
        upgrade_plugin_savepoint(true, 2026042200, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026042300) {
        upgrade_plugin_savepoint(true, 2026042300, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026042600) {
        upgrade_plugin_savepoint(true, 2026042600, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026042700) {
        upgrade_plugin_savepoint(true, 2026042700, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026042800) {
        upgrade_plugin_savepoint(true, 2026042800, 'plagiarism', 'essayguard');
    }
    ...
    // AMD triple-match tracker.js required. No DB schema changes. version.php → 2026042800.
    if ($oldversion < 2026042800) {
        upgrade_plugin_savepoint(true, 2026042800, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026042900) {
        upgrade_plugin_savepoint(true, 2026042900, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026043000) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026043000, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026043000) {
        ...
        // No DB schema changes. version.php → 2026043000.
        upgrade_plugin_savepoint(true, 2026043000, 'plagiarism', 'essayguard');
    }
    ...
    // version.php → 2026050100.
    if ($oldversion < 2026050100) {
        upgrade_plugin_savepoint(true, 2026050100, 'plagiarism', 'essayguard');
    }
    ...
    // and paste_events columns). version.php → 2026050100.
    if ($oldversion < 2026050100) {
        upgrade_plugin_savepoint(true, 2026050100, 'plagiarism', 'essayguard');
    }
    ...
    // riskscore columns that already exist). version.php → 2026050100.
    if ($oldversion < 2026050100) {
        upgrade_plugin_savepoint(true, 2026050100, 'plagiarism', 'essayguard');
    }
    ...
    // version.php → 2026050200.
    if ($oldversion < 2026050200) {
        upgrade_plugin_savepoint(true, 2026050200, 'plagiarism', 'essayguard');
    }
    ...
    // version.php → 2026050300.
    if ($oldversion < 2026050300) {
        upgrade_plugin_savepoint(true, 2026050300, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026050400) {
        upgrade_plugin_savepoint(true, 2026050400, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026051100) {
        upgrade_plugin_savepoint(true, 2026051100, 'plagiarism', 'essayguard');
    }

    // FIX-EG-SESSION-LOCK (v1.2.162): write_close() before HTTP calls in check_unlock() and auto_unlock().
    // No DB schema changes.
    if ($oldversion < 2026051300) {
        upgrade_plugin_savepoint(true, 2026051300, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026051300) {
        // ADD-EG-SIGNAL-STATUS-KEY (v1.2.181): plagiarism_essayguard_render_signal_table() now renders a
    ...
    if ($oldversion < 2026051400) {
        ...
        // No DB schema changes.
        upgrade_plugin_savepoint(true, 2026051400, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026051400) {
        ...
        //
        // Identical to the DocGuard fix (FIX-DG-PATH-A-STUB v1.0.46, savepoint 2026051400).
    ...
    if ($oldversion < 2026060400) {
        upgrade_plugin_savepoint(true, 2026060400, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026060500) {
        upgrade_plugin_savepoint(true, 2026060500, 'plagiarism', 'essayguard');
    }
    ...
    if ($oldversion < 2026072300) {
        ...
        // v1.2.219: opcache_invalidate()/opcache_reset() REMOVED.
        // opcache_reset() flushes the ENTIRE server's opcode cache — every vhost, every
        // other Moodle, every other tenant on a shared host — because one plugin changed
        // an API hostname. On a busy server that is a self-inflicted thundering herd:
        // every PHP file on the box has to be re-parsed on its next request. The targeted
        // opcache_invalidate() calls were no better founded: Moodle re-reads version.php
        // through its own upgrade machinery, and a plugin has no business reaching into
        // the process-wide opcode cache to work around a deployment that did not restart
        // PHP-FPM. If a site's opcache is stale after a plugin update, the fix is to
        // restart or reload PHP, which is the host's job, not this file's.
        upgrade_plugin_savepoint(true, 2026072300, 'plagiarism', 'essayguard');
    }
    ...
    // V1.2.219: Client-readiness release. NO DB SCHEMA CHANGES — db/install.xml is
    // unmodified, so this is a savepoint-only step. Behavioural changes that a site may
    // depend on are listed here because an upgrade is where an admin will look for them:
    // • Paste detection now actually applies on sites that never saved the settings
    // page. Scores computed before this upgrade under-report pasting; use the
    // "Rescore past attempts" tool if historical scores matter.
    // • The licence/platform-settings HTTPS calls now happen only in the new
    // 'refresh_licence' scheduled task. CRON MUST BE RUNNING.
    // • Students no longer receive a risk score or badge from the web services.
    // • report.php now honours separate-groups mode.
    // • enableplagiarism is no longer set for you — see the notice on the settings page.
